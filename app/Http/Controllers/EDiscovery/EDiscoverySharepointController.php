...
class EDiscoverySharepointController extends BaseController
{
    ...
    public function editEDiscoveryJobPage($kind, $restoreSessionId, Request $request)
    {
        ...
        $selectedSites = array_combine(array_column($selectedItems, "siteId"), $selectedItems);
        $selectedLists = array_combine(array_column($selectedItems, "listId"), $selectedItems);
        ...
        try {
            ...
            if ($ediscoveryJob->restore_point_type == 'all') {
                ...
                $restoreSession = $this->_managerVeeam->createRestoreSession($organization->veeam_organization_guid, $ediscoveryJob->restore_point_time, "vesp", $ediscoveryJob->is_restore_point_show_deleted,  $ediscoveryJob->is_restore_point_show_version)['data'];;
            } else {
                $backupJob = VeeamBackupJob::where("id", $ediscoveryJob->backup_job_id)->first();
                $restoreSession = $this->_managerVeeam->createJobRestoreSession($backupJob->guid, $ediscoveryJob->restore_point_time, 'vesp', $ediscoveryJob->is_restore_point_show_deleted,  $ediscoveryJob->is_restore_point_show_version);
            }
            //--------------------------------------//
            //----- Get Sharepoint
            $sites = $this->_managerVeeam->getSharepointSites($restoreSession->id)['data']->results;
            session()->put('restoreSharepointSessionId', $restoreSession->id);
            $sites = $this->filterItems($sites, 'asc');
            //--------------------------------------//
            //----- Get Sharepoint Lists
            foreach ($sites as $item) {
                $item->selected = false;
                $item->lists = '';
                if (optional($selectedSites)[$item->id]) {
                    if ($selectedSites[$item->id]->listId == "-1")
                        $item->selected = true;
                    else
                        $item->lists = $this->getSiteContents($item->id, $selectedLists);
                }
            }
            //---------------------------------------------//
            $arr['sites'] = $sites;
    ...
    public function moveToEdiscovery()
    {
        ...
        $requestData = json_decode(session("e-discovery_sharepoint_data"));
        $arr['job'] = (object) null;
        ...
        //---------------------------------------------//
        $selectedSites = json_decode($requestData->selectedSites);
        $selectedContents = json_decode($requestData->selectedContents);
        //---------------------------------------------//
        //----- Get Selected Job Points
        ...
        try {
            if (!session('restoreSharepointSessionId')) {
                if ($requestData->restorePointType == 'all') {
                    $organization = auth()->user()->organization;
                    $restoreSession = $this->_managerVeeam->createRestoreSession($organization->veeam_organization_guid, $requestData->jobTime, "vesp", $requestData->showDeleted,  $requestData->showVersions)['data'];;
                } else {
                    $backupJob = VeeamBackupJob::where("id", $requestData->backupJobId)->first();
                    $restoreSession = $this->_managerVeeam->createJobRestoreSession($backupJob->guid, $requestData->jobTime, 'vesp', $requestData->showDeleted,  $requestData->showVersions);
                }
            }
            $restoreSession = $this->_managerVeeam->getRestoreSession(session('restoreSharepointSessionId'))['data'];
            //--------------------------------------//
            //----- Get Sites
            $sites = $this->_managerVeeam->getSharepointSites($restoreSession->id)['data']->results;
            session()->put('restoreSharepointSessionId', $restoreSession->id);
            $sites = $this->filterItems($sites, 'asc');
            //--------------------------------------//
            $selectedSites = array_combine(array_column($selectedSites, "siteId"), $selectedSites);
            $selectedContents = array_combine(array_column($selectedContents, "contentId"), $selectedContents);
            $tempSelectedContents = array_combine(array_column($selectedContents, "siteId"), $selectedContents);
            //----- Get Sites contents
            foreach ($sites as $item) {
                $item->selected = false;
                $item->lists = '';
                ...
                if (optional($tempSelectedContents)[$item->id]) {
                    $item->lists = $this->getSiteContents($item->id, $selectedContents);
                    if (optional($selectedSites)[$item->id])
    ...
    //------------------------------------------//
    public function getSiteContents($siteId, $selectedLists)
    {
        try {
            $arr  = $this->_managerVeeam->getSiteLists(session('restoreSharepointSessionId'), $siteId)['data']->results;
            $data = [];
            foreach ($arr as $item) {
                $item->selected = false;
                $item->type = "list";
                $item->siteId = $siteId;
                if (optional($selectedLists)[$item->id])
                    $item->selected = true;
                array_push($data, $item);
            }
            $arr  = $this->_managerVeeam->getSiteLibraries(session('restoreSharepointSessionId'), $siteId)['data']->results;
            foreach ($arr as $item) {
                $item->selected = false;
                $item->type = "library";
                $item->siteId = $siteId;
    ...
    public function getEdiscoveryJobResult($kind, $id, Request $request)
    {
        ...
        if ($request->filter) {
            ...
            $tempArr = array_values(array_filter(array_map(function ($value) use ($filter) {
                return $value->siteTitle . ($value->listName ? "_" . $value->listName : "") == $filter ? $value : '';
    ...
    public function saveEDiscoveryJob(Request $request)
    {
        ...
        //---------------------------------//
        $searchData = [];
        foreach ($request->siteId as $key => $item) {
            array_push($searchData, [
                "siteId" => $item,
                "listId" => $request->listId[$key],
                "siteTitle" => $request->siteTitle[$key],
                "listName" => $request->listName[$key],
        ...
        //---------------------------------//
        dispatch(new EDiscoverySharepointBackground(auth()->user()->id, $eJob->id, $searchQueryArr, $azureData));
        //---------------------------------//
        return response()->json(['message' => __("variables.success.site")], 200);
    }
    //------------------------------------------//
