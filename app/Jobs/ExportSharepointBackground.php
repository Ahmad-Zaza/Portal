...
class ExportSharepointBackground implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if ($this->type == 'library') {
            $this->exportSiteLibraries();
        } else if ($this->type == 'folder') {
            $this->exportSiteFolders();
        } else if ($this->type == 'document') {
            $this->exportSiteDocuments();
        } else if ($this->type == 'item') {
            $this->exportSiteItems();
        }
    }
    //-------------------------------------------------------------------------//
    public function exportSiteLibraries()
    {
        ...
        try {
            ...
            foreach ($details as $item) {
                //-----------------
                $savingFunction = $this->generateStreamingCallback($item, "Exporting Sharepoint Site Libraries");
                //-- Check if Library has items
                $documents = $this->managerVeeam->getSiteDocuments($sessionId, $item->item_parent_id, $item->item_id, 0, 1);
                $folders = $this->managerVeeam->getSiteFolders($sessionId, $item->item_parent_id, $item->item_id);
                //-----------------
                if (count($documents) || count($folders)) {
                    ...
                    $item->status = "Failed";
                    $item->error_response = "No items to save.";
                    $item->save();
                }
                //-----------------
            }
            try {
                if(count($itemIds) > 0)
                    $this->managerVeeam->exportSiteLibraries($sessionId, $itemParentIds, $itemIds, $callbacks);
            } catch (Exception $e) {
                $this->historyStatus = "Failed";
                Log::log("error", "Exception While Exporting Sharepoint Site Libraries " . $e->getMessage());
            }
            //-----------------
            ...
            $history->restore_session_guid = $sessionId;
            $history->save();
            Log::log('error', 'Exception While Exporting Site Library ' . json_encode($exception->getMessage()));
        }
    }
    //-------------------------------------------------------------------------//
    public function exportSiteDocuments()
    {
        $history = BacRestoreHistory::where('id', $this->historyId)->first();
        if ($this->checkProcessCanceled($this->historyId)) {
            return;
        ...
        try {
            ...
            foreach ($details as $item) {
                //-----------------
                $savingFunction = $this->generateStreamingCallback($item, "Exporting Sharepoint Site Documents");
                //-----------------
                array_push($itemIds, $item->item_id);
                array_push($itemParentIds, $item->item_parent_id);
                array_push($callbacks, $savingFunction);
            ...
            try {
                $this->managerVeeam->exportSiteDocuments($sessionId, $itemParentIds, $itemIds, $callbacks);
            } catch (Exception $e) {
                $this->historyStatus = "Failed";
                Log::log("error", "Exception While Exporting Sharepoint Site Documents " . $e->getMessage());
            }
            //-----------------
            $history->status             = $this->historyStatus;
            $history->completion_time    = Carbon::now();
    ...
    //-------------------------------------------------------------------------//
    public function exportSiteItems()
    {
        $history = BacRestoreHistory::where('id', $this->historyId)->first();
        if ($this->checkProcessCanceled($this->historyId)) {
            return;
        ...
        try {
            ...
            foreach ($details as $item) {
                //-----------------
                $savingFunction = $this->generateStreamingCallback($item, "Exporting Sharepoint Site Items");
                //-----------------
                $attachments = $this->managerVeeam->getSiteItemsAttachments($sessionId, $item->item_parent_id, $item->item_id)['data']->results;
                if ($attachments) {
                    array_push($itemIds, $item->item_id);
                    array_push($itemParentIds, $item->item_parent_id);
                    array_push($attachmentsArr, $attachments);
                    array_push($callbacks, $savingFunction);
                } else {
                    $item->status = "No Attachments";
                    $item->save();
                    continue;
                }
                //-----------------
            ...
            try {
                if (count($itemIds) > 0) {
                    $this->managerVeeam->exportSiteItems($sessionId, $itemParentIds, $itemIds, $attachmentsArr, $callbacks);
                }
            } catch (Exception $e) {
                $this->historyStatus = "Failed";
                Log::log("error", "Exception While Exporting Sharepoint Site Items " . $e->getMessage());
            }
            //-----------------
            $history->status             = $this->historyStatus;
            $history->completion_time    = Carbon::now();
            ...
            $history->restore_session_guid = $sessionId;
            $history->save();
            Log::log('error', 'Exception While Exporting Site Items Attachments ' . json_encode($exception->getMessage()));
        }
    }
    //-------------------------------------------------------------------------//
    public function exportSiteFolders()
    {
        $history = BacRestoreHistory::where('id', $this->historyId)->first();
        if ($this->checkProcessCanceled($this->historyId)) {
            return;
        ...
        try {
            ...
            foreach ($details as $item) {
                //-----------------
                $savingFunction = $this->generateStreamingCallback($item, "Exporting Sharepoint Site Folders");
                //-----------------
                array_push($itemIds, $item->item_id);
                array_push($itemParentIds, $item->item_parent_id);
                array_push($callbacks, $savingFunction);
            ...
            try {
                $this->managerVeeam->exportSiteFolders($sessionId, $itemParentIds, $itemIds, $callbacks);
            } catch (Exception $e) {
                $this->historyStatus = "Failed";
                Log::log("error", "Exception While Exporting Sharepoint Site Folders " . $e->getMessage());
            }
            //-----------------
            $history->status             = $this->historyStatus;
            $history->completion_time    = Carbon::now();
    ...
    private function createRestoreSession($organizationId, $time, $showDeleted, $showVersions)
    {
        try {
            ...
            Log::log('error', 'Exception While Creating Restore Session  ' . $ex);
        }
    }
    //-------------------------------------------------------------------------//
    private function stopRestoreSession($sessionId)
    {
        try {
            ...
            Log::log('error', 'Exception While Stopping Restore Session  ' . $ex);
        }
    }
    //-------------------------------------------------------------------------//
