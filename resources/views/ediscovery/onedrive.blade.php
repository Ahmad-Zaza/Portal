...
@section('content')
    ...
    <div id="mainContent" class="m-0">
        ...
        <div class="row h-100">
            ...
            <div class="col-10">
                ...
                <form class="onedriveTableForm">
                    <div class="col-lg-12 left-table">
                        <div class="row" style="margin-bottom: 11px;">
                            <div class="ml-2 col-sm-12 nopadding">
                                <h5 class="txt-blue">Selected Onedrives & Folders</h5>
                            </div>
                        </div>
                        <div class="row mr-3 tableRow">
                            <table id="selectedOnedriveTable"
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('.treeSearchInput input').keyup(searchOnedrive);
            ...
            //------------------------------------------//
            $('#selectedOnedriveTable').DataTable({
                "data": [],
                ...
                'columns': [{
                    ...
                    },
                    {
                        ...
                        "render": function(data) {
                            return '' +
                                '<input type="hidden" name="onedriveId[]" value="' + data.onedriveId +
                                '">' +
                                '<input type="hidden" name="folderId[]" value="' + data.folderId +
                                '">' +
                                '<input type="hidden" name="onedriveName[]" value="' + data
                                .onedriveName + '">' +
                                '<input type="hidden" name="folderName[]" value="' + data
                                .folderName + '">' +
                                '<input type="hidden" name="url[]" value="' + data.url + '">' +
                                '<a data-onedriveId="' + data.onedriveId +
                                '" data-folderId="' + data.folderId +
                                '" title="Delete" class="deleteOnedrives"><img class="tableIcone w-13" src="/svg/Delete.svg"></a>';
                        }
                    }
                ],
                "searching": false,
                ...
                "fnDrawCallback": function(data) {
                    $('.deleteOnedrives').click(deleteOnedriveSelectedItem);
                    $('#selectedOnedriveTable_wrapper').find('.boxesCount').html($(
                        '#selectedOnedriveTable').DataTable().data().count());
                },
                "scrollY": "170px",
                "paging": false,
                "autoWidth": false,
        ...
        //----------------------------------------------------//
        function setTreeOnedrives() {
            let data = @json(optional($arr)['onedrives']);
            $('#activeclose').removeAttr('disabled');
            $("#mainTree").html("");
            $("#choose").text("Change")
            data.forEach(function(result) {
                let onedrive =
                    '<li class="has onedriveLi hand relative"><div class="relative allWidth">' +
                    '<span class="caret mailCaret '+(result.folders?"":"closeMail")+'" onclick="getOnedriveFolders(event)"></span>' +
                    '<label class="checkbox-padding-left10 checkbox-container checkbox-search">&nbsp;' +
                    '<input type="checkbox" class="onedriveCheck form-check-input" value="' +
                    ...
                    result.name + '">' +
                    result.name +
                    '</span></div><div class="folder-spinner hide"></div>'+setOnedriveFolders(result)+'</li>';

                $("#mainTree").append($(onedrive));
            });
            $("#rdate").html($(".backupDate").val());
            $("#rtime").html($(".backupTime").find(":selected").text());
            $('.tree input.onedriveCheck[type=checkbox]:not([data-parent-checked]),.tree input.onedriveFolderCheck[type=checkbox]:not([data-parent-checked])')
                .attr('data-parent-checked', false);
            $('.has.onedriveLi').has(".mailCaret:not(.closeMail)").find('> ul').fadeIn();
        }
        //----------------------------------------------------//
        function setOnedriveFolders(data){
            let onedriveId = data.id;
            if(!data.folders)
                return '';
            data = data.folders;
            let onedriveFolders = "<ul class='pt-0 pb-0 mb-0'>";
            data.forEach(function(result) {
                onedriveId = result.onedriveId;
                onedriveFolders = onedriveFolders +
                    '<li class="onedrivefolder relative"><div class="relative allWidth inline-flex">' +
                    (result.hasFolders ?
                        '<span class="caret mailCaret closeMail" onclick="getFolderChildren(event)"></span>' :
                        '') +
                    '<label class="checkbox-padding-left10 checkbox-container checkbox-search">&nbsp;' +
                    '<input type="checkbox" class="onedriveFolderCheck form-check-input" data-parentId="' +
                    (result.parentId ?result.parentId:result.onedriveId) + '" value="' +
                    result.id + '" '+(result.selected?"checked":"")+' data-parent-checked="'+(data.selected?true:false)+'"/>' +
                    '<span class="tree-checkBox check-mark-white check-mark"></span></label>' +
                    '<img class="folderIcon" src="/svg/folders/' + getFolderIcon(result.name) +
                    '.svg">' +
                    '<span onclick="getFolderChildren(event)" id="' + result.id +
                    '" data-onedriveId="' + onedriveId +
            ...
            $("#" + onedriveId).closest(".has.onedriveLi").find('ul:first').fadeToggle();
            $('.tree input.onedriveCheck[type=checkbox]:not([data-parent-checked]),.tree input.onedriveFolderCheck[type=checkbox]:not([data-parent-checked])')
                .attr('data-parent-checked', false);
            ...
            if (result.children) {
                ...
                result.children.forEach(function(result) {
                    subFolders = subFolders +
                        '<li class="onedrivefolder relative pb-0"><div class="relative allWidth inline-flex">' +
                        (result.hasFolders ?
                            '<span class="caret mailCaret closeMail" onclick="getFolderChildren(event)"></span>' :
                            '') +
                        '<label class="checkbox-padding-left10 checkbox-container checkbox-search">&nbsp;' +
        ...
        function getFolderChildren(event, item) {
            ...
            $(event.target).closest(".onedrivefolder").find('ul:first').fadeToggle();
        }
        //----------------------------------------------------//
        ...
        function treeCheckChange() {
            $(document).on('change','.tree input.onedriveCheck[type=checkbox],.tree input.onedriveFolderCheck[type=checkbox]',
                function(e) {
                    //---- Change All Children
                    $(this.closest('div')).siblings('ul').find("input[type='checkbox']").prop('checked', this.checked);
                    $(this).parentsUntil('.tree').children("input[type='checkbox']").prop('checked', this.checked);
                    $(this).closest('li').find('ul .onedriveFolderCheck').attr('data-parent-checked', $(this).prop(
                        'checked'));
                    //----------------------------------------------//
                    if (!this.checked) {
        ...
        function onedriveCheckChange() {
            //---------------------------//
            let selectedOnedrives = $('#mainTree .onedriveCheck:checked');
            let data = [];
            selectedOnedrives.each(function() {
                let id = $(this).val();
                data.push({
                    "onedriveId": id,
                    "folderId": -1,
                    "url": $(this).attr('data-url'),
                    "onedriveName": $('.has.onedriveLi #' + id).html(),
                    "folderName": ""
                });
            });
            let selectedFolders = $('#mainTree .onedriveFolderCheck:checked[data-parent-checked="false"]');
            selectedFolders.each(function() {
                let id = $(this).val();
                let onedriveId = $('.tree #' + id).attr('data-onedriveId');
                let folderName = getParentFolderName(id).substr(1);
                data.push({
                    "onedriveId": onedriveId,
                    "folderId": id,
                    "url": $('.tree [value="' + onedriveId + '"]').attr('data-url'),
                    "onedriveName": $('.tree #' + onedriveId).html(),
                    "folderName": folderName
                });
            });
            $('#selectedOnedriveTable').DataTable().clear().draw();
            $('#selectedOnedriveTable').DataTable().rows.add(data); // Add new data
            $('#selectedOnedriveTable_wrapper').find('.boxesCount').html(data.length);
            $('#selectedOnedriveTable').DataTable().columns.adjust().draw();
        }
        //----------------------------------------------------//
        function getParentFolderName(folderId) {
            if ($('#' + folderId).hasClass('mail-folder-click'))
                return getParentFolderName($('.tree .onedriveFolderCheck[value="' + folderId + '"]').attr('data-parentId')) +
                    "/" + $('#' + folderId).html()
            else
                return '';
        }
        //----------------------------------------------------//
        function deleteOnedriveSelectedItem() {
            let onedriveId = $(this).attr('data-onedriveId');
            let folderId = $(this).attr('data-folderId');
            if (folderId != -1)
                $('.tree .onedriveFolderCheck[value="' + folderId + '"]').prop('checked', false).change();
            else
                $('.tree .onedriveCheck[value="' + onedriveId + '"]').prop('checked', false).change();
        }
        //----------------------------------------------------//
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        data.data.forEach(function(result) {
                            let onedrive =
                                '<li class="has onedriveLi hand relative"><div class="relative allWidth">' +
                                '<span class="caret mailCaret closeMail" onclick="getOnedriveFolders(event)"></span>' +
                                '<label class="checkbox-padding-left10 checkbox-container checkbox-search">&nbsp;' +
                                '<input type="checkbox" class="onedriveCheck form-check-input" value="' +
                                result.id + '" data-url="' + result.url + '"/>' +
                                '<span class="tree-checkBox check-mark"></span></label>' +
                                '<span id="' + result.id +
                                '" class="mail-click left-mail-click ml-27" onclick="getOnedriveFolders(event)" data-toggle="popover" title="' +
                        ...
                        $('.tree input.onedriveCheck[type=checkbox][data-parent-checked!=true],.tree input.onedriveFolderCheck[type=checkbox][data-parent-checked!=true]')
                            .attr('data-parent-checked', false);
                        //------------------------------------------//
                        var delay = 1000,
                            setTimeoutConst;
        ...
        function getOnedriveFolders(event) {
            $target = $(event.target);
            let spinner = $target.closest('.onedriveLi.has').find('.folder-spinner:first');
            if ($target.hasClass('mailCaret')) {
                onedriveId = $target.closest('.onedriveLi.has').find('.mail-click').attr('id');
            } else {
                onedriveId = $target.attr('id');
            }
            $("#" + onedriveId).closest("div").find('.mailCaret').toggleClass('closeMail');
            if ($("#" + onedriveId).closest(".has.onedriveLi").find('ul').length) {
                $("#" + onedriveId).closest(".has.onedriveLi").find('ul:first').fadeToggle();
                return;
            }
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    res.forEach(function(result) {
                        let onedrive =
                            '<li class="has onedriveLi hand relative"><div class="relative allWidth">' +
                            '<span onclick="getOnedriveFolders(event)" class="caret mailCaret closeMail"></span>' +
                            '<label class="checkbox-padding-left10 checkbox-container checkbox-search">&nbsp;' +
                            '<input type="checkbox" class="onedriveCheck form-check-input" value="' +
                            result.id + '" data-url="' + result.url + '"/>' +
                            '<span class="tree-checkBox check-mark"></span></label>' +
        ...
        function startEdiscovery(id=-1) {
            ...
            if ($('.onedriveTableForm [name="onedriveId[]"]').length == 0)
            ...
            let onedriveData = $('.onedriveTableForm').serialize();
            ...
            $.ajax({
                ...
                data: jobData + '&' + onedriveData + "&" + conditionsData + "&_token={{ csrf_token() }}",
                success: function(data) {
                    $(".spinner_parent").css("display", "none");
                    $('body').removeClass('removeScroll');
                    //------------------------------------------//
