        ...
        <ul class="ulNavbar">
            ...
            <li class="liNavbar">
                @if (optional(optional($arr)['job'])->id)
                    <a class="active"
                        href="{{ url('e-discovery', ['edit', $arr['kind'], optional($arr)['job']->restore_session_guid]) }}">Edit
                        E-Discovery Job</a>
                @else
                    <a class="active" href="{{ url('e-discovery', ['add', $arr['kind']]) }}">New E-Discovery
                        Job</a>
                @endif
            </li>
...
@section('content')
            ...
            <div class="modal-content ">
                <div id="modalBody_id" class="modalContent">
                    <div class="alert swal-modal-confirmation custom-confirmation" role="alert">
                        ...
                        <div class="swal-title text-center confirmTitle">Editing E-Discovery Job will delete the previous
                            result</div>
                        <div class="row">
        ...
        <div class="row h-100">
            ...
            <div class="col-10">
                        ...
                        <div id="" class="float-right">
                            <form class="jobNameForm mb-0" onsubmit="jobNameFormSubmit(event)">
                                <input placeholder="Job Name" type="text"
                                    value="{{ optional(optional($arr)['job'])->name }}" name="jobName" id="jobName"
                                    required class="form_input form-control" />
                            </form>
                        </div>
                        <div class="txt-blue float-right">
                            <h5 class="mr-3 pl-3"> Job Name</h5>
                        </div>

                    </div>
                </div>

                <form class="mailboxTableForm">
                    <div class="col-lg-12 left-table">
                        <div class="row" style="margin-bottom: 11px;">
                            <div class="ml-2 col-sm-12 nopadding">
                                <h5 class="txt-blue">Selected Mailboxes & Folders</h5>
                            </div>
                        </div>
                        <div class="row mr-3 tableRow">
                            <table id="selectedMailboxTable"
                            ...
                            <div class="row m-0 pl-4 pr-4 allWidth">
                                <div class="col-lg-2 pr-0">
                                    <div class="selected-action allWidth relative">
                                        <select class="form_input form-control font-size category select2 required"
                                ...
                                <div class="col-lg-2 pr-0">
                                    <div class="selected-action allWidth relative">
                                        <select class="form_input form-control font-size conditions required"
                                            ...
                                            <option disabled value="" style="color:#999!important;">Select Condition
                                            </option>
                                ...
                                <div class="col-lg-3">
                                    <div class="selected-action allWidth flex justify-content-end">
                                        ...
                                        <button type="button" class="resetBtn btn_primary_state w-100" onclick="resetConditionsFields()">
                                            Reset</button>
                                    </div>
                                </div>
                            </div>
                ...
                <form class="conditionsTableForm">
                    <div class="col-lg-12 left-table">
                        <div class="row" style="margin-bottom: 11px;">
                            <div class="col-sm-12 ml-2 nopadding">
                ...
                <div class="row actions-row">
                    ...
                    <div class="col-lg-4 nopadding" style="margin-bottom: 25px;margin-top:25px;">
                        <button onclick="startEdiscovery({{ optional(optional($arr)['job'])->id }})"
                            class="btn_primary_state right-float">
                            Start</button>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            //------------------------------------------//
            $('#selectedMailboxTable').DataTable({
                "data": [],
                ...
                'columns': [{
                    ...
                    },
                    {
                        ...
                        "render": function(data) {
                            return '' +
                                '<input type="hidden" name="mailboxId[]" value="' + data.mailboxId +
                                '">' +
                                '<input type="hidden" name="folderId[]" value="' + data.folderId +
                                '">' +
                                '<input type="hidden" name="mailboxName[]" value="' + data
                                .mailboxName + '">' +
                                '<input type="hidden" name="folderName[]" value="' + data
                                .folderName + '">' +
                                '<input type="hidden" name="email[]" value="' + data.email + '">' +
                                '<a data-mailboxId="' + data.mailboxId +
                                '" data-folderId="' + data.folderId +
                                '" title="Delete" class="deleteMailboxes"><img class="tableIcone w-13" src="/svg/Delete.svg"></a>';
                        }
                    }
                ],
                "searching": false,
                ...
                "fnDrawCallback": function(data) {
                    $('.deleteMailboxes').click(deleteMailboxSelectedItem);
                    $('#selectedMailboxTable_wrapper').find('.boxesCount').html($(
                        '#selectedMailboxTable').DataTable().data().count());
                },
                "scrollY": "170px",
                "paging": false,
                "autoWidth": false,
            ...
            //------------------------------------------//
            $(".jobNameForm,.conditionsTableForm,.mailboxTableForm").submit(function(event) {
        ...
        function setBackupJobPoints() {
            ...
            //------------------------------------------//
            $('#showDeleted').prop("checked", "{{ optional(optional($arr)['job'])->is_restore_point_show_deleted }}" == 1 ?
                true : false)
            $('#showVersions').prop("checked", "{{ optional(optional($arr)['job'])->is_restore_point_show_version }}" == 1 ?
                true : false)
            //------------------------------------------//
        }
        //----------------------------------------------------//
        function setTreeMailboxes() {
            let data = @json(optional($arr)['mailboxes']);
            ...
            data.forEach(function(result) {
                mailbox =
                    ...
                    '<span class="caret mailCaret ' + (result.folders ? "" : "closeMail") +
                    '" onclick="getMailBoxFolders(event)"></span>' +
                    ...
                    result.id + '" data-email="' + result.email + '" ' + (result.selected ? "checked" : "") + '/>' +
                    ...
                    result.name + '">' +
                    result.name +
                    '</span></div><div class="folder-spinner hide"></div>' + setMailboxFolders(result) + '</li>';

                $("#mailboxes").append($(mailbox));
            });
            //$('.caret.mailCaret:not(.closeMail)').closest(".has.mailbox").find('ul:first').fadeToggle();
            $("#rdate").html($(".backupDate").val());
            $("#rtime").html($(".backupTime").find(":selected")
                .text());
            $('.tree input.mailBoxCheck[type=checkbox][data-parent-checked!=true],.tree input.mailBoxFolderCheck[type=checkbox][data-parent-checked!=true]')
                .attr('data-parent-checked', false);
        }
        //----------------------------------------------------//
        function setMailboxFolders(mailboxData) {
            let mailBoxId = mailboxData.id;
            if (!mailboxData.folders)
                return '';
            let data = mailboxData.folders;
            let mailboxFolders = "<ul class='pt-0 pb-0' style='display:block;'>";
            data.forEach(function(result) {
                ...
                mailboxFolders = mailboxFolders +
                    ...
                    (result.hasFolders ?
                        ...
                        '') +
                    '<label class="checkbox-padding-left10 checkbox-container checkbox-search">&nbsp;' +
                    '<input type="checkbox" class="mailBoxFolderCheck form-check-input" data-parent-checked="' + (
                        mailboxData.selected ? "true" : "false") + '" data-parentId="' +
                    (result.parentId ?? result.mailboxId) + '" value="' +
                    result.id + '" ' + (result.selected ? "checked" : "") + '/>' +
                    '<span class="tree-checkBox check-mark-white check-mark"></span></label>' +
                ...
                result.children.forEach(function(result) {
                    subFolders = subFolders +
                        ...
                        (result.hasFolders ?
                            ...
                            '') +
                        '<label class="checkbox-padding-left10 checkbox-container checkbox-search">&nbsp;' +
                        '<input type="checkbox" class="mailBoxFolderCheck form-check-input" data-parentId="' + (
                            result.parentId ?? result.mailboxId) + '" value="' +
                        result.id + '" ' + (result.selected ? "checked" : "") + '/>' +
                        '<span class="tree-checkBox check-mark-white check-mark"></span></label>' +
        ...
        function treeCheckChange() {
            $(document).on('change',
                '.tree input.mailBoxCheck[type=checkbox],.tree input.mailBoxFolderCheck[type=checkbox]',
                function(e) {
                    //---- Change All Children
                    $(this).closest('div').siblings('ul').find("input[type='checkbox']").prop('checked', this.checked);
                    $(this).parentsUntil('.tree').children("input[type='checkbox']").prop('checked', this.checked);
                    $(this).closest('li').find('ul .mailBoxFolderCheck').attr('data-parent-checked', $(this).prop(
                        'checked'));
                    //----------------------------------------------//
                    if (!this.checked) {
                        $('.tree [value="' + $(this).attr('data-parentId') + '"]').prop('checked', false);
                        $('.tree [data-parentId="' + $(this).attr('data-parentId') + '"]').attr('data-parent-checked',
                            false);
                        if ($('.tree [data-parentId="' + $(this).attr('data-parentId') + '"]').length)
                            uncheckAllParents($('.tree [data-parentId="' + $(this).attr('data-parentId') + '"]').val());
                    }
                    //----------------------------------------------//
                    mailboxCheckChange($(this));
                });
        }
        //----------------------------------------------------//
        function uncheckAllParents(item) {
            if ($('.tree [data-parentId="' + item + '"]').length > 0) {
                $('.tree [data-parentId="' + item + '"]').attr('data-parent-checked', false);
                uncheckAllParents($('.tree [data-parentId="' + item + '"]').val());
            }
        }
        //----------------------------------------------------//
        function mailboxCheckChange() {
            //---------------------------//
            let selectedMailboxes = $('#mailboxes .mailBoxCheck:checked');
            let data = [];
            selectedMailboxes.each(function() {
                let id = $(this).val();
                data.push({
                    "mailboxId": id,
                    "folderId": -1,
                    "email": $(this).attr('data-email'),
                    "mailboxName": $('.has.mailbox #' + id).html(),
                    "folderName": ""
                });
            });
            let selectedFolders = $('#mailboxes .mailBoxFolderCheck:checked[data-parent-checked="false"]');
            selectedFolders.each(function() {
                let id = $(this).val();
                let mailboxId = $('.tree #' + id).attr('data-mailboxId');
                let folderName = getParentFolderName(id).substr(1);
                data.push({
                    "mailboxId": mailboxId,
                    "folderId": id,
                    "email": $('.tree [value="' + mailboxId + '"]').attr('data-email'),
                    "mailboxName": $('.tree #' + mailboxId).html(),
                    "folderName": folderName
                });
            });
            $('#selectedMailboxTable').DataTable().clear().draw();
            $('#selectedMailboxTable').DataTable().rows.add(data); // Add new data
            $('#selectedMailboxTable_wrapper').find('.boxesCount').html(data.length);
            $('#selectedMailboxTable').DataTable().columns.adjust().draw();
        }
        //----------------------------------------------------//
        function getParentFolderName(folderId) {
            if ($('#' + folderId).hasClass('mail-folder-click'))
                return getParentFolderName($('.tree .mailBoxFolderCheck[value="' + folderId + '"]').attr('data-parentId')) +
                    "/" + $('#' + folderId).html()
            else
                return '';
        }
        //----------------------------------------------------//
        function deleteMailboxSelectedItem() {
            let mailboxId = $(this).attr('data-mailboxId');
            let folderId = $(this).attr('data-folderId');
            if (folderId != -1)
                $('.tree .mailBoxFolderCheck[value="' + folderId + '"]').prop('checked', false).change();
            else
                $('.tree .mailBoxCheck[value="' + mailboxId + '"]').prop('checked', false).change();
        }
        //----------------------------------------------------//
        ...
        function startEdiscovery(id = -1) {
            ...
            if ($('.mailboxTableForm [name="mailboxId[]"]').length == 0)
            ...
            let mailboxData = $('.mailboxTableForm').serialize();
            ...
            $.ajax({
                ...
                data: jobData + '&' + mailboxData + "&" + conditionsData + "&_token={{ csrf_token() }}",
                success: function(data) {
                    $(".spinner_parent").css("display", "none");
                    $('body').removeClass('removeScroll');
                    //------------------------------------------//
